...
import io.spine.delivery.server.given.ChainingCallStorage;
import io.spine.delivery.server.given.DirectCallStorage;
...
final class ReportingRecordStorageTest {
    ...
    void unsubscribe() {
        ...
        var firstId = TestEventId.generate();
        var firstRecord = testEventWith(firstId);

        underTest.write(firstId, firstRecord);
        subscription.cancel();

        var secondId =TestEventId.generate();
        var secondRecord = testEventWith(secondId);
